...
void Board_UART_Init(LPC_USART_T *pUART)
{
	if (pUART == LPC_USART0) {
		...
		Chip_SCU_PinMuxSet(0x2, 3, (SCU_MODE_PULLDOWN | SCU_MODE_FUNC2));				/* P1.13 : UART1_TXD */
		Chip_SCU_PinMuxSet(0x2, 4, (SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS | SCU_MODE_FUNC2));	/* P1.14 : UART1_RX */
	}
}
...
//		Chip_GPIO_SetPinState(LPC_GPIO_PORT, 1, 12, (bool) !On);
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, 11, (bool) !On);
	}
	else if (LEDNumber == 1) {
...
{
	if (LEDNumber == 0) {
		return (bool) !Chip_GPIO_GetPinState(LPC_GPIO_PORT, 0, 14);
	}
	else if (LEDNumber == 1) {
